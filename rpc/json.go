...
func (c *jsonCodec) ParseRequestArguments(argTypes []reflect.Type, params interface{}) ([]reflect.Value, Error) {
	if args, ok := params.(json.RawMessage); !ok {
		...
		if len(argTypes) == 1 {
			// TODO: maybe update this by  dec.Token() vs json.Delim('{')
			if retArgs, err := parseKeywordArguments(args, argTypes[0]); err == nil {
				return retArgs, nil
			}
		}
...
func parseKeywordArguments(rawArgs json.RawMessage, types reflect.Type) ([]reflect.Value, Error) {
	value := reflect.New(types)
	...
	if value.IsNil() && types.Kind() != reflect.Ptr {
		return args, &invalidParamsError{"missing value for required argument"}
	}
